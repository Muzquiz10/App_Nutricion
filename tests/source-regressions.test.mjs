...
test("appointment notifications require customer confirmation and immediate email", async () => {
  ...
  assert.match(source, /\+\{dayEvents\.length - 3\} más/);
  assert.match(source, /AppointmentRescheduleDialog/);
  assert.match(source, /Proponer otra fecha\/hora/);
  assert.match(source, /Cambiar fecha\/hora/);
  assert.match(source, /onReschedule/);
